...
CREATE TABLE IF NOT EXISTS `Foodboard`.`ClaimedPosts` (
  ...
  `FoodItem_Users_userID` INT NOT NULL,
  `Sessions_Users_userID` INT NOT NULL,
  PRIMARY KEY (`claimedID`, `FoodItem_itemID`, `FoodItem_Users_userID`, `Sessions_Users_userID`),
  INDEX `fk_ClaimedPosts_FoodItem1_idx` (`FoodItem_itemID` ASC, `FoodItem_Users_userID` ASC),
  INDEX `fk_ClaimedPosts_Sessions1_idx` (`Sessions_Users_userID` ASC),
  CONSTRAINT `fk_ClaimedPosts_FoodItem1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ClaimedPosts_Sessions1`
    FOREIGN KEY (`Sessions_Users_userID`)
    REFERENCES `Foodboard`.`Sessions` (`Users_userID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
