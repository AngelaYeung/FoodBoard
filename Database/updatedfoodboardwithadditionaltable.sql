...
-- -----------------------------------------------------
-- Table `Foodboard`.`ClaimedPosts`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Foodboard`.`ClaimedPosts` (
  `claimedID` INT NOT NULL AUTO_INCREMENT,
  `FoodItem_itemID` INT NOT NULL COMMENT 'itemID of claimed the claimed fooditem',
  `FoodItem_Users_userID` INT NOT NULL COMMENT 'userID associated with the posted fooditem',
  `Users_userID` INT NOT NULL COMMENT 'userID of the claimer of the fooditem',
  PRIMARY KEY (`claimedID`, `FoodItem_itemID`, `FoodItem_Users_userID`, `Users_userID`),
  INDEX `fk_ClaimedPosts_FoodItem1_idx` (`FoodItem_itemID` ASC, `FoodItem_Users_userID` ASC),
  INDEX `fk_ClaimedPosts_Users1_idx` (`Users_userID` ASC),
  CONSTRAINT `fk_ClaimedPosts_FoodItem1`
    FOREIGN KEY (`FoodItem_itemID` , `FoodItem_Users_userID`)
    REFERENCES `Foodboard`.`FoodItem` (`itemID` , `Users_userID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ClaimedPosts_Users1`
    FOREIGN KEY (`Users_userID`)
    REFERENCES `Foodboard`.`Users` (`userID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
